#!/usr/bin/env bash
# Validate CRISPR trajectory search on one generic scavenge_gpu allocation.

#SBATCH --job-name=crispr-validate
...
"$PYTHON_PATH" -m unittest discover -s crispr_trajectory_search/tests -v
"$PYTHON_PATH" -m crispr_trajectory_search.validate_backend
