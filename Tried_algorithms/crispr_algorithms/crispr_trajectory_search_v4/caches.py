...
from .config import ISLAND_NAMES
...
class AdaptiveFinishingQueues:
    """Always-on objective queues; no unreachable absolute admission threshold."""

    def __init__(self, max_size_per_island: int):
        self.max_size = max_size_per_island
        self._items: dict[str, dict[tuple[int, ...], TrajectoryEvaluation]] = {
    ...
    def update(self, evaluations: Iterable[TrajectoryEvaluation]) -> None:
        ...
        for island in ISLAND_NAMES:
            ...
            for evaluation in candidates:
                ...
                if existing is None or island_rank(evaluation, island) > island_rank(existing, island):
                    queue[key] = evaluation
    ...
    def members(self, island: str | None = None) -> list[TrajectoryEvaluation]:
        if island is not None:
            return sorted(
                self._items[island].values(),
                key=lambda item: island_rank(item, island),
                reverse=True,
            )
        combined: dict[tuple[int, ...], TrajectoryEvaluation] = {}
    ...
    def stats(self) -> dict:
        return {
            "max_size_per_island": self.max_size,
            "sizes": {
                island: len(self._items[island])
                for island in ISLAND_NAMES
            },
        }


FinishingQueue = AdaptiveFinishingQueues
