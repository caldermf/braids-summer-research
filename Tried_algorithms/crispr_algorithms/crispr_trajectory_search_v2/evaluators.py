...
class TorchTrajectoryEvaluator:
    ...
    def __init__(self, config: SearchConfig):
        ...
        verifier_config = SearchConfig(
            **{
                **config.__dict__,
                "backend": "cpu",
                "periodic_distance": config.periodic_distance,
            }
        )
    ...
    def _pattern_distance(
        ...
        """Vectorized distance to a projective monomial permutation matrix."""
        torch = self.torch
        support = state != 0
        nonzero_count = support.sum(dim=-1).to(torch.float32)
        any_support = nonzero_count > 0
        first = torch.argmax(support.to(torch.int16), dim=-1)
        last = state.shape[-1] - 1 - torch.argmax(
            torch.flip(support, dims=(-1,)).to(torch.int16),
            dim=-1,
        )
        widths = torch.where(
            any_support,
            (last - first + 1).to(torch.float32),
            torch.zeros_like(nonzero_count),
        )

        target_mask = torch.zeros(3, 3, dtype=torch.bool, device=self.device)
        rows = []
        columns = []
        ...
        off_target = ~target_mask
        penalty = (
            2.0 * nonzero_count[:, off_target]
            + 0.25 * widths[:, off_target]
        ).sum(dim=1)

        target_state = state[:, rows, columns, :]
        target_support = support[:, rows, columns, :]
        target_count = nonzero_count[:, rows, columns]
        target_width = widths[:, rows, columns]
        monomial = target_count == 1
        penalty += torch.where(
            monomial,
            torch.zeros_like(target_count),
            2.0 + 1.5 * target_count + 0.25 * target_width,
        ).sum(dim=1)

        exponents = torch.argmax(target_support.to(torch.int16), dim=-1)
        coefficients = torch.gather(
            target_state,
            dim=-1,
            index=exponents.unsqueeze(-1),
        ).squeeze(-1)
        inverse = pow(target_coefficient, -1, self.config.p)
        relative_exponents = exponents - target_exponent
        relative_coefficients = torch.remainder(
            coefficients * inverse,
            self.config.p,
        )

        has_reference = monomial.any(dim=1)
        reference_index = torch.argmax(monomial.to(torch.int16), dim=1)
        reference_exponent = torch.gather(
            relative_exponents,
            1,
            reference_index.unsqueeze(1),
        ).squeeze(1)
        reference_coefficient = torch.gather(
        ...
        exponent_difference = torch.abs(
            relative_exponents - reference_exponent.unsqueeze(1)
        ).clamp(max=20)
        coefficient_difference = (
            relative_coefficients != reference_coefficient.unsqueeze(1)
        ).to(torch.float32)
        mismatch = (
            1.0
            + 0.1 * exponent_difference.to(torch.float32)
            + 0.5 * coefficient_difference
        )
        mismatch_mask = monomial & (
            (relative_exponents != reference_exponent.unsqueeze(1))
            | (relative_coefficients != reference_coefficient.unsqueeze(1))
        )
        penalty += (mismatch * mismatch_mask.to(torch.float32)).sum(dim=1)
        penalty += (~has_reference).to(torch.float32) * 9.0
        return penalty

    def _periodic_frontier_distance(self, state):
        identity = self._pattern_distance(
            state,
            target_positions=((0, 0), (1, 1), (2, 2)),
            target_exponent=0,
            target_coefficient=1,
        )
        delta = self._pattern_distance(
            state,
            target_positions=((0, 2), (1, 1), (2, 0)),
            target_exponent=4,
            target_coefficient=self.config.p - 1,
        )
        return self.torch.minimum(identity, delta)
    ...
    def _evaluate_same_horizon(
        ...
        periodic_rows = (
            self._periodic_frontier_distance(state).cpu().tolist()
            if self.config.periodic_distance
        ...
        evaluations = [
            build_evaluation(
                ...
                periodic_distance_history=(
                    (periodic_distance,)
                    if periodic_distance is not None
                    else ()
                ),
            )
            for trajectory, history, periodic_distance in zip(
                trajectories,
                history_rows,
                periodic_rows,
            )
