...
from .edits import apply_geometry, enumerate_and_apply, sample_geometries, valid_geometries
...
def _choose_parents(
    ...
    elite_count = min(limit, max(1, round(0.70 * limit)))
    selected = ordered[:elite_count]
    selected_words = {item.factor_ids for item in selected}
    remainder = [item for item in candidates if item.factor_ids not in selected_words]
...
def generate_mutation_dataset(
    *,
    ...
    for evaluation in parent_evaluations:
        baseline_samples[evaluation.length].append(evaluation.final_projlen)
    ...
    for batch_start in range(0, len(parents), parent_batch_size):
        ...
        for (parent_index, geometry, _), child_eval in zip(
            ...
            baseline_samples[child_eval.length].append(child_eval.final_projlen)
            child_counter += 1
    ...
    baseline = LengthPercentiles.from_samples(p, n, baseline_samples)
    ...
    summary = {
        "format": "crispr-transformer-mutation-dataset-v1",
        "p": p,
        "n": n,
        ...
        "label": {
            ...
            "q": "empirical lower-tail projlen percentile conditioned on braid length",
