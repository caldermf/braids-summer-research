#!/usr/bin/env python3
"""
Monte Carlo tree search with prefix scoring and reservoir-style playouts.

This version addresses two weaknesses of the first MCTS scripts:
- every intermediate prefix in a rollout is scored, so short hits are not missed
- a node is evaluated by many sampled continuations, with a small reservoir of
  low-projective-length prefixes, rather than by one noisy rollout endpoint
...
class MonteCarloTreeSearch:
    def __init__(self, config: MCTSConfig):
        ...
        if config.playouts_per_expansion <= 0:
            raise ValueError("playouts_per_expansion must be positive")
    ...
    def create_run_directory(self) -> Path:
        ...
        run_dir = base_dir / f"mcts_reservoir_{timestamp}_seed{self.config.seed}"
        suffix = 1
    ...
    def score_prefix(self, factor_ids: List[int], burau_matrix) -> dict:
        ...
        value = self.value_from_projlen(projlen, kernel_match, len(factor_ids))
    ...
    def rollout_step(self, factor_ids: List[int], burau_matrix) -> tuple:
        ...
        next_factor_ids = factor_ids + [action]
    ...
    def reservoir_playout(self, node: MCTSNode) -> dict:
        """
        Evaluate a node by many sampled continuations.

        Every prefix encountered during every continuation is scored and placed
        into a bounded reservoir bucketed by projlen. The returned value is the
        best prefix kept in those reservoirs.
        """
        ...
        for _ in range(self.config.playouts_per_expansion):
    ...
    def value_from_projlen(self, projlen: int, kernel_match: dict, depth: int) -> float:
        if depth <= 0:
            value = 0.0
        else:
            projlen_per_length = float(projlen) / float(depth)
            value = 1.0 / (1.0 + projlen_per_length)
    ...
    def run(self) -> dict:
        ...
        for iteration in range(1, self.config.iterations + 1):
            ...
            playout_result = self.reservoir_playout(leaf)
...
def parse_args() -> MCTSConfig:
    parser = argparse.ArgumentParser(
        description="Run prefix-scoring MCTS with reservoir-style playouts."
    )
    ...
    parser.add_argument(
        "--playouts-per-expansion",
        type=int,
        default=64,
        help="Number of sampled continuations used to evaluate each expanded node",
    )
    parser.add_argument(
        "--reservoir-size",
        type=int,
    ...
    return MCTSConfig(
        ...
        playouts_per_expansion=args.playouts_per_expansion,
