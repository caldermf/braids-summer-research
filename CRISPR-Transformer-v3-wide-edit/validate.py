...
def main() -> None:
    ...
    wide_parent = automaton.sample_uniform(160, rng)
    wide_geometries = sample_balanced_geometries(
        160,
        4_096,
        rng,
        min_length=120,
    ...
    if len(wide_geometries) != 4_096:
        raise AssertionError("wide-edit candidate sampler returned the wrong size")
    if max(item.delete_length for item in wide_geometries) < 40:
        raise AssertionError("wide-edit candidate sampler missed large deletions")
    ...
    payload = {
        "status": "passed",
        "algorithm": "CRISPR-Transformer-v3-wide-edit",
        ...
        "balanced_wide_geometries_verified": len(wide_geometries),
    }
