#!/usr/bin/env bash
# Run CUDA GF(p^2) signatures and batched exact checks on scavenge_gpu.

#SBATCH --job-name=hybrid-suffix
...
mkdir -p "$REPO_ROOT/slurm_logs" "$OUTPUT_DIR/suffix-lookup"
cd "$REPO_ROOT"
