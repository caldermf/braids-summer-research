#!/usr/bin/env bash
# Export the cross-prime SQLite DB into append-only BraidLake Parquet shards.
# Submit from braids-summer-research.

#SBATCH --job-name=lake-global
...
"$PYTHON_PATH" -u -m braid_lake.export_sqlite global-db \
  --db "$GLOBAL_DB" \
  --lake-root "$LAKE_ROOT" \
  --manifest "$MANIFEST" \
  --p "$P" \
