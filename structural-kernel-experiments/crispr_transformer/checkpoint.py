...
def load_checkpoint(path: str | Path) -> tuple[dict, list[Candidate]]:
    ...
    if payload.get("format") not in {
        "paper-tracker-reservoir-run-v1",
        "paper-tracker-reservoir-run-v2",
        "datta-defect-stratified-reservoir-v1",
        "commutator-reservoir-checkpoint-v2",
    }:
    ...
    for index, item in enumerate(payload["candidates"]):
        ...
        if "factor_ids" in item:
            factor_ids = tuple(int(value) for value in item["factor_ids"])
        else:
            factor_ids = tuple(
        ...
        candidates.append(
            Candidate(
                ...
                matrix_fingerprint=item.get("matrix_fingerprint", ""),
                source_index=index,
...
def load_checkpoints(paths: list[str | Path]) -> tuple[dict, list[Candidate]]:
    ...
    for path in paths:
        ...
        identity = (
            int(metadata["p"]),
            int(metadata["n"]),
            int(metadata.get("r", 1)),
            str(metadata.get("objective", "ordinary_projlen")),
            metadata.get("generator_index"),
        )
        if common is None:
            common = identity
