#!/usr/bin/env bash
# Directly grow exact words from an exhaustive BraidZero frontier.
# No suffix bank and no collision oracle.
# Submit from the braids-summer-research directory.

#SBATCH --job-name=braidzero-grow
...
ROLLOUTS_PER_FRONTIER="${ROLLOUTS_PER_FRONTIER:-1}"
GROWTH_MODE="${GROWTH_MODE:-random}"
...
RUN_GROUP="${RUN_GROUP:-p${P}_frontier${FRONTIER_LENGTH}_direct_grow_len${TARGET_LENGTH}}"
...
"$PYTHON_PATH" -u -m braidzero.frontier_grow \
  ...
  --rollouts-per-frontier "$ROLLOUTS_PER_FRONTIER" \
  --growth-mode "$GROWTH_MODE" \
  --action-samples "$ACTION_SAMPLES" \
